...
public class ContextMapper implements Serializable {
	...
	public ContextMapper(sem.graph.SemanticGraph graph, ArrayList<String> verbalForms, ArrayList<String> nounForms, boolean interrogative){
	...
	public void integrateAllContexts(){
		...
		var roleGraphNodes = graph.getRoleGraph().getNodes();
		...
		for (SemanticNode<?> node : graph.getRoleGraph().getNodes()){
			...
			if (graph.getRoleGraph().getInNeighbors(node).isEmpty()){
				...
				var contextGraphNodes = graph.getContextGraph().getNodes();
		...
		//
			// if there are no "root" nodes at all (all nodes have incoming edges), then there is probably a copula verb with a relative clause 
				...
				for (SemanticEdge ed : graph.getRoleGraph().getInEdges(node)){
					...
					if (ed.getLabel().equals("rstr")){ // restriction
						predNode = graph.getStartNode(ed); // verb -> beautiful
						// sonst wird das (relative clause) wegfallen, ohne dieses resriction
						break;
					}
				}
		...
		if (conGraph.getNodes().isEmpty() && predNode != null){ // check if not empty
		...
		var test_dep_edge_close = depGraph.getEdges(); // ctx
		for (var edge : test_dep_edge_close) {
			if (edge.getLabel().contains("without")) {
				String secondVerb = edge.getDestVertexId();
				SemanticNode<?> secondVerbNode = getNodeAfterName(secondVerb);
				if (secondVerbNode != null && !nounForms.contains(((SkolemNodeContent) secondVerbNode.getContent()).getPosTag()))
					addSelfContextNodeAndEdgeToGraph(secondVerbNode, true); // !!
			}
		}
		for (SemanticNode<?> cNode: conGraph.getNodes()){
			// if no incoming edges, it is the top context

			var neigh = conGraph.getInEdges(cNode);
			if (conGraph.getInEdges(cNode).isEmpty()){ // ломается вот здесь
				noOfTops ++;
				...
				if (noOfTops > 1){
					...
					for (SemanticEdge edge : graph.getContextGraph().getEdges(cNode)){
						...
						var endnode = graph.getFinishNode(edge);
						var graphConEdges = conGraph.getEdges();
						graph.addContextEdge(ctxEdge, firstTop, graph.getFinishNode(edge));
					}
					listOfTops.add(cNode);
					continue;
				}
				var edgesDependence = depGraph.getEdges(); // ctx
				for (var edge : edgesDependence)
				{
					if (edge.getLabel().contains("without"))
					{
//						setContextsForWithoutCase();
						String firstVerb  = edge.getSourceVertexId();
						String secondVerb = edge.getDestVertexId();
						SemanticNode<?> firstVerbNode  = getNodeAfterName(firstVerb);
						SemanticNode<?> secondVerbNode = getNodeAfterName(secondVerb);
						var firstVerbInNeighbours  = depGraph.getInNeighbors(firstVerbNode);
						var secondVerbInNeighbours = depGraph.getInNeighbors(secondVerbNode);
						if (!firstVerbInNeighbours.isEmpty() && firstVerbInNeighbours.contains(secondVerbNode))
						{
							// means first depends on second
							String nodeContextNameMainVerb      = "ctx(" + secondVerb  + ")";
							String nodeContextNameDependentVerb = "ctx(" + firstVerb + ")";
							SemanticNode<?> ctxNodeMainVerb      = getNodeAfterName(nodeContextNameMainVerb);
							SemanticNode<?> ctxNodeDependentVerb = getNodeAfterName(nodeContextNameDependentVerb);
							ContextHeadEdge labelEdge = new ContextHeadEdge(GraphLabels.ANTIVER, new RoleEdgeContent());
							graph.addContextEdge(labelEdge, ctxNodeMainVerb, ctxNodeDependentVerb);
						}
						if (!secondVerbInNeighbours.isEmpty() && secondVerbInNeighbours.contains(firstVerbNode))
						{
							// means second depends on first
							String nodeContextNameMainVerb      = "ctx(" + firstVerb  + ")";
							String nodeContextNameDependentVerb = "ctx(" + secondVerb + ")";
							SemanticNode<?> ctxNodeMainVerb      = getNodeAfterName(nodeContextNameMainVerb);
							SemanticNode<?> ctxNodeDependentVerb = getNodeAfterName(nodeContextNameDependentVerb);
							ContextHeadEdge labelEdge = new ContextHeadEdge(GraphLabels.ANTIVER, new RoleEdgeContent());
							graph.addContextEdge(labelEdge, ctxNodeMainVerb, ctxNodeDependentVerb);
	...
	private SemanticNode<?> getNodeAfterName(String nodeName)
	{
		Set<SemanticNode<?>> allNodes = graph.getGraph().getNodes();
		for (SemanticNode<?> node : allNodes)
		{
			String nameOfNode = node.getLabel();
			if (nameOfNode.equals(nodeName))
			{
				return node;
			}
	...
	private void setContextsForWithoutCase()
	{
		...
		for (var edge : edgesDependence) {
			if (edge.getLabel().contains("without")) {
				...
				{
					ContextHeadEdge labelEdge = new ContextHeadEdge(GraphLabels.ANTIVER, new RoleEdgeContent());
//			graph.addContextEdge(labelEdge, firstVerbNode, secondVerbNode);
//			graph.addContextEdge(labelEdge, ctxNodeMainVerb, ctxNodeDependentVerb); //
				}
				if (!secondVerbInNeighbours.isEmpty() && secondVerbInNeighbours.contains(firstVerbNode))
				{
					// means second depends on first
					if (secondVerbNode != null)
						addSelfContextNodeAndEdgeToGraph(secondVerbNode, true); // !!
					String nodeContextNameFirstVerb  = "ctx(" + firstVerb  + ")";
					String nodeContextNameSecondVerb = "ctx(" + secondVerb + ")";
					SemanticNode<?> ctxNodeFirstVerb  = getNodeAfterName(nodeContextNameFirstVerb);
					SemanticNode<?> ctxNodeSecondVerb = getNodeAfterName(nodeContextNameSecondVerb);
					ContextHeadEdge labelEdge = new ContextHeadEdge(GraphLabels.ANTIVER, new RoleEdgeContent());
					graph.addContextEdge(labelEdge, ctxNodeFirstVerb, ctxNodeSecondVerb); // !!! top=> anti => ctx(leave) => ctx => leave; top => ctx => close
				}
			}
		}
	 ...
	 * Case 1: there is an implcative verb and its embedded verb is negated, e.g. She knew that he had not arrived yet. = er ist erwartet aber noch nicht da
	 ...
	 */
	// case 2 = she didnt' know that he was coming
...
//		 remove the negation node
	...
	private void integrateNotContexts(SemanticNode<?> node, SemanticNode<?> negNode, SemanticNode<?> previousNegNode){
		...
		SemanticNode<?> head = null; // negation an sich (not ...)
		// initialize the context of the head of the negation
		SemanticNode<?> ctxHead = null; // das konetext von negierten verb
		// get the dep head
		if (depGraph.getInNeighbors(node).size() == 0)
			return;
		...
		// the boy is not singin and takling
		if (coordCtxs.containsKey(depHead) && coordCtxs.get(depHead).equals("verbal")){
			...
			if (disjunction == true){
				...
				ctxHead = graph.getContextGraph().getInNeighbors(ctxOfDepHead).iterator().next(); // kombined context head fur zB or
				/*
			 ...
			 * should be removed and a new edge from the top to the negated context should be created.
			 */
			// the boy is not walking and the girl is not talking
			...
			// create the self node of the negation
			negNode = addSelfContextNodeAndEdgeToGraph(node, false);
			// methode kreiert kontext (come) and fugt koncept of come als kontext head
			...
			// create the self node of the head of the negation
			ctxHead = addSelfContextNodeAndEdgeToGraph(head, true); // head = verb das negiert wird
		}
		...
		negCtxs.put(negNode, head); // neg node = negiertes kontext, gebraucht fur andere falle auch
...
//					|| ((SkolemNodeContent) node.getContent()).getStem().equals("nearly")
			...
			else if (((SkolemNodeContent) node.getContent()).getStem().equals("almost")
					|| ((SkolemNodeContent) node.getContent()).getStem().equals("nearly")){
				integrateAlmostContexts( node, negNode, previousNegNode);
//				}
			}

			else if (((SkolemNodeContent) node.getContent()).getStem().equals("without")) {
				integrateWithoutContexts( node, negNode, previousNegNode);
			}

			else if (((SkolemNodeContent) node.getContent()).getStem().equals("about")
			      || ((SkolemNodeContent) node.getContent()).getStem().equals("near") )
			{
				integrateNearContext(node, negNode, previousNegNode);
			}
	...
	private void integrateImplicativeContexts(){
		...
		// ----
		Set<SemanticNode<?>> nod = graph.getRoleGraph().getNodes();
		// ----
	...
	private boolean dependsOnVP(SemanticNode<?> node)
	{
		SemanticNode<?> depHead = depGraph.getInNeighbors(node).iterator().next();
		return verbalForms.contains(((SkolemNodeContent) depHead.getContent()).getPosTag());
	}



	private void integrateWithoutContexts(SemanticNode<?> node, SemanticNode<?> negNode, SemanticNode<?> previousNegNode)
	{

		// wenn von VB - die gleiche implementierung wie das VP almost und wenn nicht dann nichts machen
		SemanticNode<?> head = null; // negation an sich (not ...)
		// initialize the context of the head of the negation
		SemanticNode<?> ctxHead = null; // das konetext von negierten verb
		// get the dep head
		SemanticNode<?> depHead = depGraph.getInNeighbors(node).iterator().next();
		var inNeighbours = depGraph.getInNeighbors(node);
		// check to see if it is a copula verb being negated, e.g. The man is not beautiful.
		// in this case, the negated term ("beautiful") should be handled as a verb for
		// finding the head variable of the negated node
		var neighbours = depGraph.getInNeighbors(node);
		var neighboursOfNeighbour = depGraph.getInNeighbors(neighbours.iterator().next());
		boolean negOnCopula = false;
		for (SemanticEdge outEdge : depGraph.getOutEdges(depHead)) {
			if (outEdge.getLabel().equals("cop")){
				negOnCopula = true;
		...
		if (coordCtxs.containsKey(depHead) && coordCtxs.get(depHead).equals("verbal")){
			...
			// create the self node of the head of the negation
		//	if (node != graph.getRootNode())
//				ctxHead = addSelfContextNodeAndEdgeToGraph(head, true); // head = verb das negiert wird
		}
		if (nounForms.contains(((SkolemNodeContent) depHead.getContent()).getPosTag())) {
			ctxHead = addSelfContextNodeAndEdgeToGraph(head, true); // head = verb das negiert wird

			ContextHeadEdge labelEdge  = new ContextHeadEdge(GraphLabels.VER, new RoleEdgeContent());
			ContextHeadEdge labelEdge2 = new ContextHeadEdge(GraphLabels.ANTIVER, new RoleEdgeContent());

			graph.addContextEdge(labelEdge, negNode, ctxHead);
			graph.addContextEdge(labelEdge2, ctxHead, depHead);

			// put the negated node created and its head to the hash
			negCtxs.put(negNode, head);
			negCtxs.put(ctxHead, depHead);
...
// Implementation for VP is in IntegrateAllContexts() function

	}


	private void changeNodeForNearContext(SemanticNode<?> node)
	{
//		var depEdges = depGraph.getEdges(node);
		var edges = depGraph.getEdges(node);
		boolean hasBe         			   = false;
		boolean hasParticiple              = false;
		SemanticNode<?> nodeWithParticiple = null;
		for (var edge : edges)
		{
			var edgeLabel = edge.getLabel(); // aux / nsubj / ...
			var dependentWord = getNodeAfterName(edge.getDestVertexId());
			if(dependentWord != null)
			{
				if (edgeLabel.equals("aux") && ((SkolemNodeContent) dependentWord.getContent()).getStem().equals("be"))
					hasBe = true;
				else if (edgeLabel.equals("dep") && ((SkolemNodeContent) dependentWord.getContent()).getPosTag().equals("VBN"))
				{
					hasParticiple = true;
					nodeWithParticiple = getNodeAfterName(edge.getDestVertexId());
				}
			}
		}
		if (hasBe && hasParticiple && nodeWithParticiple != null)
			{

				var testGraph = new sem.graph.SemanticGraph();
				var u = 8;
//				this.graph = new sem.graph.SemanticGraph();

				graph.setRootNode(nodeWithParticiple);

				String sourceParticipleId = nodeWithParticiple.getId();
				var edgesForNode      = graph.getGraph().getEdges(node);
				var edgesForPariciple = graph.getGraph().getEdges(nodeWithParticiple);
				for (var edge : edgesForNode)
				{
					if (edge.getDestVertexId().equals(nodeWithParticiple.getId()))
					{
						edge.setDestVertexId(node.getId());
						edge.setSourceVertexId(nodeWithParticiple.getId());
						continue;
					}
					edge.setSourceVertexId(nodeWithParticiple.getId());
				}
	...
	private void integrateNearContext(SemanticNode<?> node, SemanticNode<?> negNode, SemanticNode<?> previousNegNode)
	{

		// if about == rootNode && aux=[about, be] && dep=[about, VBN]
		// change about root to VBN
...
//			}
		...
		SemanticNode<?> ctxHead = null; // das konetext von negierten verb
		// get the dep head
		var x = depGraph.getInNeighbors(node);
		if (!depGraph.getInNeighbors(node).iterator().hasNext())
			return;
		SemanticNode<?> depHead = depGraph.getInNeighbors(node).iterator().next(); // !! nullptr
		// check to see if it is a copula verb being negated, e.g. The man is not beautiful.
		// in this case, the negated term ("beautiful") should be handled as a verb for
		// finding the head variable of the negated node
		boolean negOnCopula = false;
		...
		if (coordCtxs.containsKey(depHead) && coordCtxs.get(depHead).equals("verbal")){
			...
			// first if to deal with none negation
			if (!verbalForms.contains(((SkolemNodeContent) depHead.getContent()).getPosTag()) && negOnCopula == false){
				if (depGraph.getInNeighbors(depHead).size() != 0) {
					SemanticNode<?> headOfHead = depGraph.getInNeighbors(depHead).iterator().next();
					head = headOfHead;
				}
			} else {
	...
	private void integrateAlmostContexts(SemanticNode<?> node, SemanticNode<?> negNode, SemanticNode<?> previousNegNode) {
		// if the word modifies the verb - integrateNotContexts, must have an own context
		// if a noun - ? must be a normal graph
		...
		{
//			if (!hasQuantifierNeighbours(node)) {
			ContextHeadEdge labelEdge = new ContextHeadEdge(GraphLabels.CTX_RSTR, new RoleEdgeContent());
			graph.addContextEdge(labelEdge, negNode, depHead);
			if (head instanceof SkolemNode)
				((SkolemNodeContent) head.getContent()).setContext(ctxHead.getLabel());
			setContextsRecursively(head, head);
//		}
	...
	private ArrayList<String> getQuantifiers()
	{
		ArrayList<String> quantifiers = new ArrayList<String>();
		quantifiers.add("all"  );
		quantifiers.add("fewer");
		quantifiers.add("fewest");
		quantifiers.add("few");
		quantifiers.add("least");
		quantifiers.add("less");
		quantifiers.add("little");
		quantifiers.add("many");
		quantifiers.add("more");
...
//		quantifiers.add("almost");
	...
	boolean hasQuantifierNeighbours(SemanticNode<?> node)
	{
		boolean foundQuantifier = false;
		var inNeighbours = depGraph.getInNeighbors(node);
		for (SemanticNode<?> nodeIn : inNeighbours)
		{
			var partOfSpeech = ((SkolemNodeContent) nodeIn.getContent()).getPartOfSpeech();
			if (partOfSpeech.equals("NN"))
			{
				Set<SemanticNode<?>> setForParent = depGraph.getOutNeighbors(nodeIn);
				for (var nodeFromParent : setForParent)
				{
					if (getQuantifiers().contains(((SkolemNodeContent) nodeFromParent.getContent()).getSurface()))
					{
